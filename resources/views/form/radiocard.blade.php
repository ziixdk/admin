@include("admin::form._header")
        ...
        <div class="flex flex-wrap gap-1" role="group">
            @foreach($options as $option => $label)
                <input type="radio" name="{{$name}}" value="{{$option}}" id="{{$name}}-{{$option}}" class="sr-only peer/opt {{$class}}" {{ ($option == old($column, $value)) || ($value === null && in_array($label, $checked)) ?'checked':'' }} {!! $attributes !!} />
